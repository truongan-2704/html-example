...
class Dynamic_conv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bias is not None:
            ...
            output = torch.nn.functional.conv2d(x, weight=aggregate_weight, bias=aggregate_bias, stride=self.stride, padding=self.padding,
                              dilation=self.dilation, groups=self.groups * batch_size)
        else:
            output = torch.nn.functional.conv2d(x, weight=aggregate_weight, bias=None, stride=self.stride, padding=self.padding,
                              dilation=self.dilation, groups=self.groups * batch_size)
